...
(function(){
  ...
  function injectGlobalButtons() {
    ...
    // WhatsApp number — use SITE_WA from api.js if available
    var waNum = (typeof SITE_WA !== 'undefined') ? SITE_WA : '923021807669';

    // Cart count from localStorage
    function getCartCount() {
      try {
        var raw = localStorage.getItem('cpc_cart');
        var arr = raw ? JSON.parse(raw) : [];
        return Array.isArray(arr) ? arr.reduce(function(s,i){ return s+(i.quantity||0); },0) : 0;
      } catch(_) { return 0; }
    }
    ...
    wrap.innerHTML =
      '<button id="g-float-cart" class="g-float-cart" aria-label="View cart">' +
        '<span class="g-cart-icon">&#128722;</span>' +
        '<span id="g-cart-badge" class="g-cart-badge">0</span>' +
      '</button>' +
    ...
    function syncCartBadge() {
      ...
      var badge = document.getElementById('g-cart-badge');
      var btn   = document.getElementById('g-float-cart');
      if (badge) badge.textContent = count;
      if (btn)   btn.classList.toggle('visible', count > 0);
    }
    syncCartBadge();

    // Sync on storage changes (cross-tab) and periodically
    window.addEventListener('storage', function(e){ if(e.key==='cpc_cart') syncCartBadge(); });
    setInterval(syncCartBadge, 2000);

    // Cart button behaviour
    var cartBtn = document.getElementById('g-float-cart');
    if (cartBtn) {
      cartBtn.addEventListener('click', function(e) {
        e.preventDefault();
        // On menu page — open existing drawer
        if (typeof openCart === 'function') {
          openCart();
        } else {
          // On other pages — navigate to menu
